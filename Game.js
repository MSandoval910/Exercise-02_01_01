...
var Asteroid = (function()
{
	...
	var create = function()
	{
		var obj = Object.create(def);
		obj.radius = 40;
        // changing the color of the asteroids in the game
		obj.color = "blue";
		obj.pos = Vec2D.create(0, 0);
		obj.vel = Vec2D.create(0, 0);
		obj.blacklisted = false;
...
function generateThrustParticle()
{
    ...
    // changing the color of th eparticle trail on the ship
	p.color = 'pink';
	p.lifeSpan = 80;
...
function generateAsteroidExplosion(asteroid)
{
	...
	for(i; i > -1; --i)
	{
		...
		p.lifeSpan = 80;
        // paticle color for the asteroids when they are destroyed by the ships bullets
		p.color = 'green';
		p.vel.setLength(20 / p.radius);
		p.vel.setAngle(ship.angle + (1 - Math.random() * 2) * doublePI);
...
function render()
{
    // changing the color of all of the background of the page
...
function renderBullets()
{
	...
	for(i; i > -1; --i)
	{
		...
		context.arc(b.pos.getX() >> 10, b.pos.getY() >> 10, b.radius, 10, doublePI);
		if(Math.random() > 100) context.stroke();
		context.closePath();
	}
}
...
function renderScanlines()
{
	...
	for(i; i > -1; --i)
	{
		...
		context.strokeStyle = (Math.random() > 0.0001) ? 'green' : 'purple';
		context.stroke();
	}
...
function generateShot()
{
	...
	b.radius = 100;
	b.pos.setXY(ship.pos.getX() + Math.cos(ship.angle) * 200, ship.pos.getY() + Math.sin(ship.angle) * 200);
	b.vel.setLength(1000);
